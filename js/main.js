...
function editComment(commentId) {
    ...
    const body = Array.from(card.children).find(el => el.classList?.contains('comment-body'));
    if (!body || Array.from(body.children).some(el => el.classList?.contains('comment-edit-form'))) return;
    ...
    const commentText = Array.from(body.children).find(el => el.classList?.contains('comment-text'));
    const commentActions = Array.from(body.children).find(el => el.classList?.contains('comment-actions'));
    ...
    if (commentActions) body.insertBefore(form, commentActions);
    ...
    form.addEventListener('submit', async event => {
        ...
        try {
            const result = await authRequest(`/api/comments/${encodeURIComponent(commentId)}`, {comment: nextComment}, 'PUT', true);
            ...
            showToast(error?.message || 'Could not update comment.', 'error');
        }
    });
}
